(function(){
  ...
  const CAT_SPOTS = [
    { id:0, name:'Попкинс Чуча', emoji:'🐈\u200d⬛', place:'спит в домике из подушек', placeEmoji:'🛌', starter:true, cost:0, sleeping:true },
    { id:1, name:'Котик', emoji:'🐱', place:'на лежанке', placeEmoji:'🛋️', cost:30 },
    { id:2, name:'Котик', emoji:'🐱', place:'в коробке', placeEmoji:'📦', cost:60 },
    { id:3, name:'Котик', emoji:'🐱', place:'на окне', placeEmoji:'🪟', cost:100 },
    { id:4, name:'Котик', emoji:'🐱', place:'в корзинке', placeEmoji:'🧺', cost:150 },
    { id:5, name:'Котик', emoji:'🐱', place:'на когтеточке', placeEmoji:'🌳', cost:220 },
    { id:6, name:'Котик', emoji:'🐱', place:'в гамаке', placeEmoji:'🪢', cost:300 },
  ];
  const MAX_CATS = CAT_SPOTS.length;
  const CAT_TICK_MS = 5 * 60 * 1000; // 5 минут
  const CAT_TICK_REWARD = 10;
  ...
  /* ================= CAT FARM ================= */
  function costFor(spot){ return spot.cost; }
  ...
  function renderCats(){
    ...
    CAT_SPOTS.forEach(spot=>{
      ...
      el.className = 'cat-slot' + (spot.starter?' starter':'') + (!owned?' locked':'') + (spot.sleeping&&owned?' sleeping':'');
      ...
      if(owned){
        ...
        inner += `<div class="cat-name">${spot.name}</div>`;
        inner += `<div class="place-label">${spot.place}</div>`;
        inner += `<div class="progress-bar"><div class="progress-fill" data-bar="${spot.id}"></div></div>`;
        inner += `<div class="progress-note">+10💗 раз в 5 мин</div>`;
      } else {
        inner += `<div class="cat-emoji">🔒</div>`;
        inner += `<div class="place-label">${spot.place}</div>`;
        inner += `<button class="btn tiny adopt-btn" data-adopt="${spot.id}">Приручить · 💗${spot.cost}</button>`;
      }
      el.innerHTML = inner;
    ...
    catSlotsEl.querySelectorAll('[data-adopt]').forEach(btn=>{
      btn.addEventListener('click', ()=> adoptCat(Number(btn.dataset.adopt)));
    });
  ...
  function updateProgressBars(){
    ...
    unlockedCats.forEach(id=>{
      const bar = catSlotsEl.querySelector(`[data-bar="${id}"]`);
      if(!bar) return;
  ...
  function maybeShowTutorial(){
    ...
    const starterEl = catSlotsEl.querySelector('.cat-slot.starter');
    if(!starterEl || starterEl.querySelector('.tutorial-bubble')) return;
    const bubble = document.createElement('div');
    bubble.className = 'tutorial-bubble';
    bubble.innerHTML = `Мяу! Я <b>Попкинс Чуча</b> 🐈\u200d⬛<br>Погладь меня — получишь +1💗. А ещё я сама приношу +10💗 раз в 5 минут.<br>Собери 100💗 и открой свою первую крутку удачи ✦<span class="tb-close">поняла, мур!</span>`;
  ...
  renderInventory();
  updateUI();
  if(hadSave){ showSaveToast('С возвращением! Прогресс загружен ✓'); }
})();
